...
def model_parameters(parser_nn):
  ...
  parser_nn.add_argument(
      ...
      type=float,
      default=0.0,
      help='Percentage of data dropped in convolutional layers',
  )
  parser_nn.add_argument(
      '--gru_units',
      type=str,
  ...
  parser_nn.add_argument(
      ...
      type=float,
      default=0.0,
      help='Percentage of data dropped in recurrent GRU layers',
  )
  parser_nn.add_argument(
      '--dropout1',
      type=float,
      default=0.1,
      help='Percentage of data dropped in dense hidden layers',
  )
  parser_nn.add_argument(
      '--units1',
      type=str,
...
def model(flags):
  ...
  for filters, kernel_size, activation, dilation_rate, strides in zip(
    ...
    net = stream.Stream(
        cell=tf.keras.layers.Conv2D(
            ...
            strides=strides,
            kernel_regularizer=l2_reg))(net)
            
    # Apply dropout after each CNN layer if configured
    if dropout_cnn > 0:
      net = tf.keras.layers.Dropout(rate=dropout_cnn)(net)
  ...
  dropout_recurrent = getattr(flags, 'dropout_recurrent', 0.0)
  ...
  for units, return_sequences in zip(
    ...
    net = gru.GRU(
        ...
        recurrent_dropout=dropout_recurrent)(net)
  ...
  for units, activation in zip(
    ...
    # Apply dropout after every Dense hidden layer
    if flags.dropout1 > 0:
      net = tf.keras.layers.Dropout(rate=flags.dropout1)(net)

  net = tf.keras.layers.Dense(units=flags.label_count, kernel_regularizer=l2_reg)(net)
